import {
  ...
  // Removed old imports - products and posts tables no longer exist
  
  // Note: auth.users table already exists in Supabase
  // We'll reference it directly when needed without defining it here
  ...
  export const profiles = pgTable("profiles", {
    profile_id: uuid()
      .primaryKey(),
      // Note: In production, this should reference auth.users.id
    avatar: text(),
    name: text().notNull(),
  ...
  export const notifications = pgTable("notifications", {
    ...
    // Removed product_id and post_id references - those tables no longer exist
    // TODO: Add quest_id and tower_id references if needed for gamification notifications
    target_id: uuid()
